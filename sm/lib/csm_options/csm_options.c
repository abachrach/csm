...
const char * csm_options_banner_string = "";

void csm_options_banner(const char*message) {
	csm_options_banner_string = message;
}
...
int csm_options_parse_args(struct csm_option*ops, int argc, const char* argv[]) {
	...
	for (i=1; i<argc; i++) {
		...
		if(!strcmp("config_dump", name)) {
			csm_options_dump(ops, stdout, 0);
			exit(0);
		}
		
		if(!strcmp("help",name) || !strcmp("h",name) ) {
			csm_options_print_help(ops, stdout);
			exit(0);
		}
		...
		if(!strcmp("config", name)) {
			if(i>=argc-1) {
				...
				if(!csm_options_tolerant) return 0;
			}
			if(!csm_options_parse_file(ops, ".", argv[i+1])) {
				if(!csm_options_tolerant) return 0;
			}
			i++;
			continue;
		...
		struct csm_option * o;
		if(0 == (o =csm_options_find(ops, name)) ) {
			fprintf(stderr, "Option '%s' not found (use -help to get list of csm_options).\n", name);
			if(!csm_options_tolerant) return 0;
		}

		csm_options_set_passed(o);
		if(csm_options_requires_argument(o)) {
			if(i>=argc-1) {
				fprintf(stderr, "Argument %s needs value.\n", o->name);
				if(!csm_options_tolerant) return 0;
			}
			if(!csm_options_set(o, argv[i+1])) {
				if(!csm_options_tolerant) return 0;
			}
			i++;
		} 
...
int csm_options_parse_stream(struct csm_option*ops, const char*pwd, FILE*file) {
	...
	while(fgets(linesto, MAX_LINE_LENGTH-1, file)) {
		...
		if(*line == '<') { line++;
			while(isspace(*line)) line++;
			if(!csm_options_parse_file(ops, pwd, line)) {
				if(!csm_options_tolerant) return 0;
			}
			continue;
		}
		...
		if(!csm_options_try_pair(ops, name, value) && !csm_options_tolerant) {
			return 0;
		}
	}
	return 1;
...
int csm_options_parse_file(struct csm_option*ops, const char*pwd, const char*filename) {
	...
	/* free(resolved); */
	return csm_options_parse_stream(ops, newdir, file);
}
...
struct csm_option * csm_options_find(struct csm_option*ops, const char * name) {
	int j;
	for(j=0;csm_options_valid(ops+j);j++)
...
int csm_options_try_pair(struct csm_option*ops, const char*name, const char*value) {
	...
	return csm_options_set(o, value);
}

int csm_options_valid(struct csm_option*o) {
	return o->name != 0;
}

int csm_options_set(struct csm_option*o, const char*value) {
	switch(o->type) {	
		case(OPTION_INT): {
...
/*			fprintf(stderr, 
		...
		case(OPTION_ALTERNATIVE): {
			int * value_pointer = (int*) o->value_pointer;
			struct csm_option_alternative * a = o->alternative;
			for(; a->label; a++) {
...
const char*csm_options_value_as_string(struct csm_option*o);
...
void csm_options_dump(struct csm_option * csm_options, FILE*f, int write_desc) {
	int n; for (n=0;csm_options_valid(csm_options+n);n++);
	...
	int i;
	for (i=0;i<n;i++) {
		table[row*3 +0] = strdup_(csm_options[i].name);
		table[row*3 +1] = strdup_(csm_options_value_as_string(csm_options+i));
		table[row*3 +2] = write_desc ? strdup_(csm_options[i].desc) : strdup_("");

		if( write_desc)
		if(csm_options[i].type == OPTION_ALTERNATIVE) {
			char extended[1000];
			strcat(extended, csm_options[i].desc);
			strcat(extended, "  Possible csm_options are: ");
	
			struct csm_option_alternative * a = csm_options[i].alternative;
			for(; a->label; a++) {
				strcat(extended, "\"");
...
void csm_options_print_help(struct csm_option * csm_options, FILE*f) {
	fprintf(f, csm_options_banner_string);
	fprintf(f, 
	"Generic csm_options: \n"
	...
	csm_options_dump(csm_options, f, 1);
}

static char csm_options_value_as_string_buf[1000];
const char*csm_options_value_as_string(struct csm_option*o) {
	if(!o->value_pointer) {
		return "NULL";
	}
	...
	switch(o->type) {
		case(OPTION_INT): {
			int * value_pointer = (int*) o->value_pointer;
			sprintf(csm_options_value_as_string_buf, "%d", *value_pointer);
			break;
		}

		case(OPTION_STRING): {
			char** value_pointer = (char**) o->value_pointer;
			sprintf(csm_options_value_as_string_buf, "%s", *value_pointer); /* should I add "" ? */
			break;
		}
		
		case(OPTION_DOUBLE): {
			double * value_pointer = (double*) o->value_pointer;
			sprintf(csm_options_value_as_string_buf, "%g", *value_pointer);
			break;
		}
		...
		case(OPTION_ALTERNATIVE): {
			...
			for(; a->label; a++) {
				if( a->value == *value_pointer )
					sprintf(csm_options_value_as_string_buf, "%s", a->label);
			}
			break;
		}
		
		default: 
		strcpy(csm_options_value_as_string_buf, "?");
	} /* switch */	
	
	return csm_options_value_as_string_buf;
}
